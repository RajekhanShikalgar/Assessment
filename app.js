...
async function checkAuthStates() {
  ...
  // Strict Tab/Window Session Isolation:
  // sessionStorage is destroyed when a tab/window is closed.
  // If neither token is present, the user has closed the tab or logged out.
  let adminTok = null;
  let teacherTok = null;
  try {
    adminTok = sessionStorage.getItem('ciems_admin_token');
  ...
  if (!adminTok && !teacherTok) {
    ...
    if (tLoginBox) tLoginBox.classList.remove('hidden');
    if (tDashView) tDashView.classList.add('hidden');

    const aLoginBox = document.getElementById('admin-login-box');
    const aDashView = document.getElementById('admin-dashboard-view');
    if (aLoginBox) aLoginBox.classList.remove('hidden');
    if (aDashView) aDashView.classList.add('hidden');

    // Invalidate any lingering server-side cookie in the background
    try {
  ...
  // Check Teacher only if teacherTok exists in sessionStorage
  if (teacherTok) {
    try {
  ...
  // Check Admin only if adminTok exists in sessionStorage
  if (adminTok) {
    try {
